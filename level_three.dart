...
import 'package:tic/level_four.dart';
...
class LevelThreePage extends StatefulWidget {
  ...
  @override
  _LevelThreePageState createState() => _LevelThreePageState();
}
...
class _LevelThreePageState extends State<LevelThreePage> {
  ...
  void resetGame() {
    setState(() {
      board = List<String>.filled(boardSize * boardSize, '');
      backgroundColors = List<Color>.filled(boardSize * boardSize, const Color(0xFFD6EAF8));
      currentPlayer = 'X';
      winner = null;
    });
  }
  ...
  void aiMoveLevelTwo() {
    ...
    if (move != null) {
      setState(() {
        ...
        backgroundColors[move] = currentPlayer == 'X' ? Colors.blue.withOpacity(0.5) : Colors.red.withOpacity(0.5);
        winner = calculateWinner();
  ...
  int? findBestMove() {
    ...
    List<int> preferredMoves = getPreferredMoves();
  ...
  List<int> getPreferredMoves() {
    // Generate preferred moves based on board size
    List<int> preferredMoves = [];
    int center = boardSize * boardSize ~/ 2;
    if (boardSize % 2 != 0) {
      preferredMoves.add(center); // Center
    }
    for (int i = 0; i < boardSize; i++) {
      for (int j = 0; j < boardSize; j++) {
        if ((i == 0 || i == boardSize - 1) && (j == 0 || j == boardSize - 1)) {
          preferredMoves.add(i * boardSize + j); // Corners
        } else if (i == 0 || i == boardSize - 1 || j == 0 || j == boardSize - 1) {
          preferredMoves.add(i * boardSize + j); // Edges
        }
      }
    }
  ...
  String? calculateWinner() {
    List<List<int>> lines = [];
    for (int i = 0; i < boardSize; i++) {
      lines.add(List.generate(boardSize, (index) => i * boardSize + index)); // Rows
      lines.add(List.generate(boardSize, (index) => index * boardSize + i)); // Columns
    }
    lines.add(List.generate(boardSize, (index) => index * boardSize + index)); // Diagonal
    lines.add(List.generate(boardSize, (index) => (index + 1) * boardSize - (index + 1))); // Anti-diagonal
