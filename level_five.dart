...
import 'package:tic/level_one.dart';
...
class LevelFivePage extends StatefulWidget {
  ...
  @override
  _LevelFivePageState createState() => _LevelFivePageState();
}
...
class _LevelFivePageState extends State<LevelFivePage> {
  ...
  void handleTap(int index) {
    ...
    setState(() {
      ...
      if (winner != null) {
        ...
        if (currentPlayer == 'O') {
          Future.delayed(_animationDuration, aiMoveLevelThree);
        }
      }
    });
  }
  ...
  List<int> getPreferredMoves() {
    ...
    if (boardSize % 2 != 0) {
      preferredMoves.add(center);
    }
    for (int i = 0; i < boardSize; i++) {
      for (int j = 0; j < boardSize; j++) {
        if ((i == 0 || i == boardSize - 1) && (j == 0 || j == boardSize - 1)) {
          preferredMoves.add(i * boardSize + j);
  ...
  String? calculateWinner() {
    List<List<int>> lines = [];
    for (int i = 0; i < boardSize; i++) {
      lines.add(List.generate(boardSize, (index) => i * boardSize + index));
      lines.add(List.generate(boardSize, (index) => index * boardSize + i));
    }
    lines.add(List.generate(boardSize, (index) => index * boardSize + index));
    lines.add(List.generate(boardSize, (index) => (index + 1) * boardSize - (index + 1)));
